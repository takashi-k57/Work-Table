...
<div class="admin_index">
    <div class="month">
        <a class="btn btn-primary" href="/admin?year={{$last_month->year}}&month={{$last_month->month}}"
            role="button">&lt;前月</a>
        <p>{{$current_month->year}}年{{$current_month->month}}月</p>
        <a class="btn btn-primary" href="/admin?year={{$following_month->year}}&month={{$following_month->month}}"
            role="button">翌月&gt;</a>
    </div>
    ...
    <table border="1">
        ...
        <tr>
            ...
            <td>
                @for ($i = 1; $i <= $current_month->daysInMonth; $i++)
                    @if ($weekday%7 == 0)
            <td class="sunday">{{$weekdays[($weekday++)%7]}}</td>
            @elseif (array_search($current_month->format('Y-m') . '-' . sprintf('%02d', $i), $isHolidays))
            <th class="holiday">{{$weekdays[($weekday++)%7]}}</th>
            @else
            ...
            @for ($i = 1; $i <= $current_month->daysInMonth; $i++)
                ...
                <td class="sunday">
                    <button type="button" class="btn btn-success" data-toggle="modal" data-target="#modalForm"
                        data-user_id="{{$user->id}}" data-worksystem="{{$user->worksystem}}"
                        data-day="{{$current_month->format('Y-m') . '-' . sprintf('%02d', $i)}}">
                ...
                <div class="modal fade" id="modalForm" role="dialog">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <form role="form" action="{{ action('Admin\AdminCalendarController@store') }}"
                                method="POST">
                                <!-- Modal ボディー -->
                                @csrf
                            ...
                            <div class="modal-footer">
                                <form role="form" action="{{ action('Admin\AdminCalendarController@delete') }}"
                                    method="POST">
                                    @csrf
                                    {{method_field('DELETE')}}
                                    <div>
